...
import models_DA.modules
import models_DA.modules.Permutations
from models_DA.modules import flow, thops, FlowAffineCouplingsAblation
from utils.load_models import opt_get
...
class FlowStep(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels,
        ...
        # 1. actnorm
        self.actnorm = models_DA.modules.FlowActNorms.ActNorm2d(in_channels, actnorm_scale)

        # 2. permute
        if flow_permutation == "invconv":
            self.invconv = models_DA.modules.Permutations.InvertibleConv1x1(
                in_channels, LU_decomposed=LU_decomposed)

        # 3. coupling
        if flow_coupling == "CondAffineSeparatedAndCond":
            self.affine = models_DA.modules.FlowAffineCouplingsAblation.CondAffineSeparatedAndCond(in_channels=in_channels,
                                                                                                opt=opt)
